...
| T-033 | F-Droid / FOSS Flavor CI Verification | CI step: `./gradlew :app:dependencies --configuration fossReleaseRuntimeClasspath | grep -i "gms\|play-services\|mlkit"` must show only the allowlisted bundled ML Kit barcode-scanning subtree. ABI split config for `foss` | In: CI. Out: — | Spec §16.1 | CI workflow file | T-001 | Yes | DevOps | S (0.5d) | Low | dependency check matches are limited to ML Kit barcode scanning and its transitive support artifacts | `.github/workflows/`, `build.gradle.kts` |
...
- `./gradlew :app:dependencies --configuration fossReleaseRuntimeClasspath | grep -i "gms\|play-services\|mlkit"` matches only the allowlisted bundled ML Kit barcode-scanning subtree.
